...
/*	
	...
	*/
    ...
    public String insertTrainer(){
		boolean morePokemon = true;
		String answer = "";
		int pokemonCount = 0;

		try{
		//collects Trainer information
		Scanner scan = new Scanner(System.in);
		System.out.print("Enter the name of the Trainer you would like to insert: ");
		String trainer = scan.nextLine();
		System.out.print("\nEnter the Trainer's type: ");
		String trainerType = scan.nextLine();
		
		/*Insert into Trainer table*/

		do {
			System.out.print("\nEnter the ID of one of the Trainer's pokemon: ");
			int trainerPoekmonID = scan.nextInt();
			//System.out.print("\nEnter the name of one of the Trainer's pokemon: ");
			String trainerPokemonName = scan.nextLine();
			/*will have to perform a selection, if it return 0 then perform insertPokemon
				and insert into Owns,
				else just add to Owns table*/
			s.executeQuery ("SELECT " + trainerPoekmonID + ", pokemonName FROM Pokemon");
			ResultSet rs = s.getResultSet();
			int count = 0;

			while (rs.next()) {
			//checks if the pokemon is already in the database
				trainerPokemonName = rs.getString("pokemonName");
				++count;
			}
			if(count > 0) {
			//inserts the Trainer's pokemon into the Owns table
				s.executeUpdate("INSERT INTO Owns VALUES ('" + trainer + "', '" + trainerPokemonName + "', " + trainerPoekmonID + ")");
				++pokemonCount;
			}
			else {
			//does not allow the insertion of a pokemon is not already in the database
				System.out.println("This Pokemon does not currently exist in the Pokedex");
			}
			//reset count for the Trainer's next pokemon
			count = 0;
			//incase the trainer has more than one pokemon
			System.out.print("\nDoes your Trainer have more Pokemon (y/n): ");
			answer = scan.next();
			if(answer != "y"){
				morePokemon = false;
			}
		...
		//if the Trainer has at least one pokemon, then he/she is a valid Trainer
		if (pokemonCount > 0) {
			s.executeUpdate("INSERT INTO Trainer VALUES('" + trainer + "', '" + trainerType + "')");
			return "Successful Trainer insertion";
		}
		return "Unsuccessful Trainer insertion";
		}
		catch(SQLException e){
			System.err.println ("Error message: " + e.getMessage ());
			System.err.println ("Error number: " + e.getErrorCode ());
